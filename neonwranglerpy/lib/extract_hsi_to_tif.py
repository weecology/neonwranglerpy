...
def tile_solar_angle(full_path):
    ...
    which_paths = np.unique(hdf5_file[sitename][
        "Reflectance/Metadata/Ancillary_Imagery/Data_Selection_Index"][()])
    solar_angle = []
    for pt in which_paths:
        # if pt is negative, get any from the available to avoid error(the pixel is blank
        #  anyway)
        if pt < 0:
...
def generate_raster(h5_path, save_dir, rgb_filename=None, bands="no_water",
    ...
    array2raster(solar_tilename, sol_sens_angle, metadata, clipExtent,
                 save_dir, invert_axes=False)
